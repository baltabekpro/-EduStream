 ...
 * 
 * Usage: node teacher-usage-test.js
 */
    ...
    if (registerResponse.status === 201 || registerResponse.status === 200) {
      ...
      if (loginResponse.status === 200 && loginResponse.data.token) {
        return {
          ...
          apiResponse: { hasToken: true, token: loginResponse.data.token, user: loginResponse.data.user },
          timestamp
        };
      } else {
        return {
  ...
  try {
    ...
    if (createResponse.status === 201 || createResponse.status === 200) {
      ...
      if (listResponse.status === 200 && Array.isArray(listResponse.data)) {
        const courseExists = listResponse.data.some((c) => c.id === courseId);
...
/**
 * Test 3: Material Upload - Check materials endpoint
 */
async function testMaterialUpload(token, courseId) {
  const timestamp = new Date().toISOString();
 ...
 */
async function testAIWorkspace(token, materialId) {
  const timestamp = new Date().toISOString();
 ...
 */
async function testOCRFunctionality(token) {
  const timestamp = new Date().toISOString();
 ...
 */
async function testDashboard(token, courseId) {
  const timestamp = new Date().toISOString();
 ...
 */
async function testAnalytics(token, courseId) {
  const timestamp = new Date().toISOString();
...
async function runAllTests() {
  ...
  const token = authResult.apiResponse && authResult.apiResponse.token 
    ? authResult.apiResponse.token
    : '';
  ...
  if (token) {
    ...
    if (courseResult.status === 'success' && courseResult.apiResponse && courseResult.apiResponse.courseId) {
